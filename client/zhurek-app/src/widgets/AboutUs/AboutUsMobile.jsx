...
export const AboutUsMobile = () => {
  ...
  return (
    <div className='relative min-h-[100vh] w-full flex flex-col h-full overflow-hidden'>
      {/* Block 1 */}
      <div className="relative flex h-[55vh] flex-col md:flex-row">
        <div>
          <img
            src={abstractCircle}
            alt=""
            className="absolute -top-[10%] left-1/2 -translate-x-1/2 w-[500px] h-[500px] z-0 transform scale-y-[-1] brightness-[1] sepia-[0.2] saturate-[1.2] from-yellow-100 via-white to-transparent pointer-events-none"
          />
        </div>
        <div className="z-10 w-full h-full flex flex-col justify-center gap-y-10 text-center p-[10px]">
          <p className=' text-[35px] font-bold text-[#4F3422]'>{t('hero.intelligent')}</p>
          <p className=' text-[35px] font-bold text-[#4F3422]'>{i18n.language === 'kk' ? t('hero.mentalHealth') : t('hero.chatbot')}</p>
          <p className=' text-[35px] font-bold text-[#4F3422]'>{i18n.language === 'kk' ? t('hero.chatbot') : t('hero.mentalHealth')}</p>
        </div>
      </div>
      <div>
        <p className='text-gray-500 text-center'>{t('hero.experience')}</p>
        <p className='text-gray-500 text-center'>{t('hero.subtitle')}</p>
      </div>

      {/* Block 2 */}
      <div className="flex flex-col gap-x-[50px] justify-between items-center bg-[#4F3422] text-white rounded-[40px] my-10 mx-5 px-10 py-14">
        {/* Left: Text + Features */}
